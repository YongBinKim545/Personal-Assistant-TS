    ...
    <div>
        ...
        <div @dragover.prevent.stop="dragging = true" @dragleave.prevent.stop="dragging = false" @drop.prevent="onDrop"
            ...
            <label for="file-upload"
                class="w-full cursor-pointer border-2 border-dashed rounded-lg"
                :class="dragging ? 'border-primary opacity-50': 'border-light-border dark:border-dark-border'">
                <div class="p-5 h-[120px] flex justify-center items-center text-md" id="drop_zone">
                    ...
                    <span>&nbsp Drag & Drop or Click to Select</span>
                </div>
                <input id="file-upload" type="file" class="hidden" @change="onChange" />
            </label>
        </div>
    </div>
</template>
